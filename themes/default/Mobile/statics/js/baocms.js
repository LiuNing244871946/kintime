...
function change_user_mobile(url1,url2){
		...
		layer.open({
			...
			skin: 'layui-layer-demo', //加上边框
			area: ['100%', '2.8rem'], //宽高
			content: '<div class="form"><div id="bm_m"><select id="areacode"><option value="86">中国+86</option><option value="856">老挝+856</option></select><input name="mobile" id="mobile" type="text" placeholder="请输入手机号"/> <input type="button""  id="jq_send" value="获取验证码"/></div><div id="bm_c"><span class="bm_text">验证码：</span><input  name="yzm" id="yzm" type="text" placeholder="请输入验证码"/></div><div id="bm_b"><input type="submit" value="立刻认证" id="go_mobile" /></div></div>'
		});
	//获取验证码
        var mobile_timeout;
        var mobile_count = 100;
        ...
        $(function () {
            $("#jq_send").click(function () {
                if (mobile_lock == 0) {
                    ...
                    $.ajax({
                        url: url1,
                        data: 'mobile='+document.getElementById('areacode').value+$("#mobile").val(),
                        type: 'post',
                        success: function (data) {
                            if (data.status == 'success') {
                                mobile_count = 60;
		...
		//提交
		$('#go_mobile').click(function(){
			var ml = document.getElementById('areacode').value+$('#mobile').val();
			var y = $('#yzm').val();
			$.post(url2,{mobile:ml,yzm:y},function(result){										
				if(result.status == 'success'){
