...
function setUid($uid) {
    ...
    cookie('KinTime_TOKEN', $uid, 86400 * 365); //存一年
    return true;
}

function clearUid() {
    cookie('KinTime_TOKEN', null); //存一年
    return true;
}

function getUid($token=null) {
    import("ORG/Crypt/Base64");
    $token = $token ? $token :  urldecode(cookie('KinTime_TOKEN'));
...
function is_app(){
     //return true;
     return strpos($_SERVER['HTTP_USER_AGENT'], 'KinTimeApp');
}

function is_android(){
    //return true;
     return strpos($_SERVER['HTTP_USER_AGENT'], 'KinTimeAppAndroid');
}
...
/**
 * 判断输入的字符串是否是一个合法的返利比例
 *
 * @param string $string
 * @return boolean
 */
function isRebate($string) {
    if (preg_match('/^\d{0,3}$/', $string))
        return true;
    return false;
}
 ...
 * @return boolean
 */
function isQprice($string) {
    if (preg_match('/^\d{1,5}+[.]+\d{1,6}$/', $string))
        return true;
    return false;
}
...
function isMobile($string) {
    // if(mb_strlen($string,'utf-8')===13){
    //     if (preg_match('/^[861]+[3,4,5,7,8]+\d{9}$/', $string))
    //         return true;
    //     return false;
    // }else if (mb_strlen($string,'utf-8')===14) {
    //     if (preg_match('/^\d{13}$/', $string))
    //         return true;
    //     return false;
    // };

    if(mb_strlen($string,'utf-8')===13){
        if (preg_match('/^[8]+[6]/', $string)){
            if (preg_match('/^[8]+[6]+[1]+[3,4,5,7,8]+\d{9}$/', $string))
                return true;
            return false;
        }else if(preg_match('/^[8]+[5]/', $string)){
            if (preg_match('/^\d{3}+[2]+[0]/', $string))
                return true;
            return false;
        }
    }else if (mb_strlen($string,'utf-8')===14) {
        if (preg_match('/^\d{3}+[0]+[2]+[0]/', $string))
            return true;
        return false;
    }else{
        return false;
