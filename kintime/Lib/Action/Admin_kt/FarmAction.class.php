...
class FarmAction extends CommonAction {
   ...
   public function _initialize() {
        ...
        if ($this->isPost()) {
            ...
            if ($farm_id = $obj->add($data)) {
                ...
                foreach($group_id as $key=>$val){
                    D('FarmGroupAttr')->add(array('mer_id'=>$data['mer_id'],'attr_id'=>$val));
                }
                foreach($cate_id as $k=>$v){
                    D('FarmPlayAttr')->add(array('mer_id'=>$data['mer_id'],'attr_id'=>$v));
                }
    ...
    private function createCheck() {
        ...
        $data = $this->checkFields($this->_post('data', false), array('mer_id', 'farm_name','intro', 'tel', 'photo', 'addr', 'city_id', 'area_id', 'business_id','price','lat', 'lng', 'business_time', 'details','notice','environmental', 'have_room', 'have_washchange', 'have_wifi', 'have_shower', 'have_tv', 'have_ticket', 'have_toiletries', 'have_hotwater'));
        ...
        }elseif(!$shop = D('merchants')->find($data['mer_id'])){
            $this->baoError('商家不存在');
        }
        ...
        if ($farm_id = (int) $farm_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    D('FarmGroupAttr')->where(array('mer_id'=>$data['mer_id']))->delete();
                    foreach($group_id as $key=>$val){
                        D('FarmGroupAttr')->add(array('mer_id'=>$data['mer_id'],'attr_id'=>$val));
                    }
                    D('FarmPlayAttr')->where(array('mer_id'=>$data['mer_id']))->delete();
                    foreach($cate_id as $k=>$v){
                        D('FarmPlayAttr')->add(array('mer_id'=>$data['mer_id'],'attr_id'=>$v));
                    }
                    $this->baoSuccess('操作成功', U('farm/index'));
                ...
                $cate_id = M('FarmGroupAttr')->where(array('mer_id'=>$detail['mer_id']))->select();
                $group_id = M('FarmPlayAttr')->where(array('mer_id'=>$detail['mer_id']))->select();
                ...
                $this->assign('shop',D('merchants')->find($detail['mer_id']));
