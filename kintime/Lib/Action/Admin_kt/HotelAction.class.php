...
class HotelAction extends CommonAction {
    ...
    public function index() {
        ...
        if ($keyword = $this->_param('keyword', 'htmlspecialchars')) {
            $map['hotel_name|eng_hotel_name|lao_hotel_name'] = array('LIKE', '%' . $keyword . '%');
        ...
        $list = $hotel->where($map)->order(array('orderby' => 'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    private function createCheck() {
        $data = $this->checkFields($this->_post('data', false), array('hotel_name','mobile','areacode','lao_hotel_name','eng_hotel_name','mer_id','addr','lao_addr','eng_addr', 'city_id', 'area_id','business_id','cate_id', 'type','price','star', 'tel', 'details', 'lao_details','eng_details','photo', 'lng', 'lat','is_wifi','is_kt','is_nq','is_tv','is_xyj','is_ly','is_bx','is_base','is_tooth','is_rsh','in_time','out_time','rate','orderby'));
        ...
        }elseif(!$merchants = D('Merchants')->find($data['mer_id'])){
            $this->baoError('商家不存在');
        }
    ...
    private function editCheck() {
        $data = $this->checkFields($this->_post('data', false), array('hotel_name','mobile','areacode','lao_hotel_name','eng_hotel_name','mer_id','lao_addr','eng_addr','addr', 'city_id', 'area_id','business_id','cate_id', 'type','price','star', 'tel', 'details', 'lao_details', 'eng_details', 'photo', 'lng', 'lat','is_wifi','is_kt','is_nq','is_tv','is_xyj','is_ly','is_bx','is_base','is_tooth','is_rsh','in_time','out_time','rate','orderby'));
        $data['hotel_name'] = htmlspecialchars($data['hotel_name']);
        $data['eng_hotel_name'] = htmlspecialchars($data['eng_hotel_name']);
        $data['lao_hotel_name'] = htmlspecialchars($data['lao_hotel_name']);
        if (empty($data['hotel_name'])) {
    ...
    public function refuse($hotel_id){
        $obj = D('Hotel');
         if (is_numeric($hotel_id) && ($hotel_id = (int) $hotel_id)) {
                $obj->save(array('hotel_id' => $hotel_id, 'audit' => 2));
                $this->baoSuccess('操作成功！', U('hotel/index'));
         }
    }
