...
class UsercashAction extends CommonAction {
    ...
    public function index() {
        ...
        $begin_time = I('begin_time','','htmlspecialchars,trim');
        $end_time = I('begin_time','','htmlspecialchars,trim');
        $this->assign('begin_time',$begin_time);
        $this->assign('end_time',$end_time);
        $begin_time = strtotime($begin_time);
        $end_time = strtotime($end_time);
        if($begin_time && $end_time){
            $map['addtime'] = array('between', $begin_time.','.$end_time);
        }
        ...
        foreach ($list as $key => $row) {
            ...
            $count_money += $row['money'];
        }
        $this->assign('count_money', $count_money); // 赋值数据集
