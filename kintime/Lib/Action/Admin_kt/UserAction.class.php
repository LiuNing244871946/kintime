...
class UserAction extends CommonAction {
	...
	public function recovery ($user_id){
		if ($user_id = (int) $user_id) {
			...
			$data1['closed'] = 0;
			if (false !==$obj->save($data1)) {
				$this->baoSuccess('操作成功', U('user/index'));
			}
			$this->baoError('操作失败');
        } else {
            $this->baoError('请选择要还原的会员');
        }
	}
    ...
    public function delete($user_id = 0) {
	 ...
	 public function rdelete($user_id = 0) {
        if (is_numeric($user_id) && ($user_id = (int) $user_id)) {
            $obj = D('Users');
            $objwx = D('connect');
            $connect_id = $objwx->where(array('uid'=>$user_id))->getField('connect_id');
            if($connect_id){
                    $objwx->delete($connect_id);
            }
            $obj->save(array('user_id'=>$user_id,'closed'=>1));
            $obj->delete($user_id);
            $this->baoSuccess('删除成功！', U('user/index'));
    ...
    public function qualification(){
       ...
       if($this->isPost()){
            $qualification = (int)  $this->_post('qualification');
            $initial = (int)  $this->_post('initial');
            $available = (int)  $this->_post('available');
            if($qualification == 0){
                if($detail['available']!=$detail['initial']){
                    $this->baoError('该用户还有账单未还！');
                }
                ...
                $data['available'] = $initial - $detail['initial'] + $detail['available'];
            }
            $data['user_id'] = $user_id;
    ...
    function weixin(){
        ...
        $map['uid'] = array('neq','');
        if($account = $this->_param('account','htmlspecialchars')){
            $user_map['account'] = array('LIKE','%'.$account.'%');
            $user = D('Users')->where($user_map)->select();
            foreach($user as $val){
                $map['uid'] = $val['user_id'];
            }
            $this->assign('account',$account);
        ...
        $count = D('Connect')->where($map)->count(); // 查询满足要求的总记录数
        $Page = new Page($count, 25);
        $show = $Page->show(); // 分页显示输出
        $connects = D('Connect')->where($map)->order(array('connect_id'=>'asc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
        foreach($connects as $k=>$v){
            $connects[$k]['user'] = D('Users')->find($v['uid']);
        }
    ...
    function weixin_delete($connect_id){
        if (is_numeric($connect_id) && ($connect_id = (int) $connect_id)) {
            $obj = D('Connect');
            $obj->delete($connect_id);
            $this->baoSuccess('删除成功！', U('user/weixin'));
        } else {
            if (is_array($connect_id)) {
                $obj = D('Connect');
                foreach ($connect_id as $id) {
                    $obj->delete($id);
                }
                $this->baoSuccess('删除成功！', U('user/weixin'));
            }
            $this->baoError('请选择要删除的条目');
        }
    }
}
