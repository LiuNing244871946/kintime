...
class EleorderAction extends CommonAction {
    ...
    public function index() {
        ...
        foreach ($ele as $key => $value) {
            $ele2[$value['shop_id']] = $value;
        }
        foreach ($list as $k => $val) {
            $list[$k]['package_money'] = $ele2[$val['shop_id']]['package_money'];  
            $val['package_money'] = $ele['package_money'];
    ...
    public function delete($order_id = 0) {
     ...
     public function fanxian($order_id = 0) {
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            $obj = D('Eleorder');
            $detail = $obj->find($order_id);
            if ($detail['closed'] == 1 && $detail['is_pay'] == 1) {
                $intro = 'dingdan：'. $detail['order_id'] .'取消退款';
                D('Users')->addMoney($detail['user_id'],$detail['need_pay'] ,$intro);
                $obj->save(array('order_id' => $order_id, 'status' => 10));
            }else{
              $this->baoSuccess('退款失败！', U('eleorder/index')); 
            }
            //$obj->save(array('order_id' => $order_id, 'closed' => 1));
