...
class AreaAction extends CommonAction {
    ...
    private $create_fields = array('area_name','lao_area_name','eng_area_name','city_id', 'orderby');
    private $edit_fields = array('area_name','lao_area_name','eng_area_name','city_id', 'orderby');
    ...
    public function index() {
        ...
        if($keyword){
            $map['area_name|lao_area_name|eng_area_name'] = array('LIKE', '%'.$keyword.'%');
        }    
    ...
    private function createCheck() {
        ...
        if (empty($data['area_name'])) {
            $this->baoError('中文区域名称不能为空');
        } 
        $data['lao_area_name'] = htmlspecialchars($data['lao_area_name']);
        if (empty($data['lao_area_name'])) {
            $this->baoError('老文区域名称不能为空');
        } 
        $data['eng_area_name'] = htmlspecialchars($data['eng_area_name']);
        if (empty($data['eng_area_name'])) {
            $this->baoError('英文区域名称不能为空');
        } 
    ...
    public function child($city_id = 0){
        $datas = D('Area')->fetchAll();
        $str ='<option value="0">请选择</option>';
        foreach($datas as $val){
            if($val['city_id'] == $city_id){
                $str.='<option value="'.$val['area_id'].'">'.$val['area_name'].'</option>';                
            }            
        }
        echo $str;die;
    }
