...
class ShopnewsAction extends CommonAction {
    ...
    private $create_fields = array('mer_id', 'title', 'photo', 'details', 'views', 'orderby');
    private $edit_fields = array('mer_id', 'title', 'photo', 'details', 'views', 'orderby');
    ...
    public function index() {
       ...
       if($cate_id = (int)$this->_param('cate_id')){
        ...
        if($mer_id = (int)$this->_param('mer_id')){
            $map['mer_id'] = $mer_id;
            $shop = D('merchants')->find($mer_id);
            $this->assign('mer_name',$shop['mer_name']);
        ...
        foreach($list as $k=>$val){
            if($val['mer_id']){
                $mer_ids[$val['mer_id']] = $val['mer_id'];
            }
            $val['create_ip_area'] = $this->ipToArea($val['create_ip']);
            $list[$k] = $val;
        }
        if($mer_ids){
            $this->assign('shops',D('merchants')->itemsByIds($mer_ids));
        }
    ...
    private function createCheck() {
        ...
        $shop = D('merchants')->find($data['mer_id']);
        if (empty($shop)) {
            $this->baoError('请选择正确的商家');
        }
        $map = array('mer_id' => $data['mer_id']);
        $city_id = D('merchants')->where($map)->getField('city_id');
