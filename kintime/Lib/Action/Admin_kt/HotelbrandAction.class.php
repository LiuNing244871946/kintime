...
class HotelbrandAction extends CommonAction {
    ...
    private function createCheck() {
        $data = $this->checkFields($this->_post('data', false), array('title','eng_title','lao_title', 'orderby'));
        $data['title'] = htmlspecialchars($data['title']);
        if (empty($data['title'])) {
            $this->baoError('品牌中文名称不能为空');
        }
        $data['eng_title'] = htmlspecialchars($data['eng_title']);
        if (empty($data['eng_title'])) {
            $this->baoError('品牌英文名称不能为空');
        }
        $data['lao_title'] = htmlspecialchars($data['lao_title']);
        if (empty($data['lao_title'])) {
            $this->baoError('品牌老文名称不能为空');
        }
