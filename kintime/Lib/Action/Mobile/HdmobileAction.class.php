...
class HdmobileAction extends CommonAction {
    ...
    public function loaddata() {
        ...
        $map = array('audit' => 1, 'closed' => 0, 'city_id' => $this->city_id,'time' => array('EGT', TODAY));
    ...
    public function sign($huodong_id) {
        ...
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                ...
                $this->baoSuccess('恭喜您报名成功', U('hdmobile/index'),U('hdmobile/index'));
            }
            $this->baoMsg('操作失败！');
        } else {
            $this->assign('detail', $detail);
            $this->mobile_title = '活动报名';
            $this->display();
        }
    }
    ...
    public function send_message() {
        ...
        if (IS_AJAX) {
			...
			if($Usermessage = $obj->where(array('content'=>$content,'uid'=>$user_id))->select()){
				 $this->ajaxReturn(array('status' => 'error', 'message' => '不能重复提交'));
			}
    ...
    public function hdfabu() {
        ...
        if ($this->isPost()) {
			...
			if($Huodong = $obj->where(array('title'=>$data['title'],'user_id'=>$data['user_id']))->select()){
				 $this->baoMsg('不能重复提交');
			}
