...
class PaymentAction extends CommonAction {
    ...
    public function payment($log_id) {
        ...
        if($logs['code'] == 'weixin' || $logs['code'] == 'alipay'){
            $logs['need_pay'] = (string) ($logs['need_pay'] *$this->qp);
        }
        $this->assign('button', D('Payment')->getCode($logs));
        $this->assign('types', D('Payment')->getTypes());
        $this->assign('lao_types', D('Payment')->getLaoTypes());
        $this->assign('eng_types', D('Payment')->getEngTypes());
        $this->assign('logs', $logs);
        if($lang == 1){
            $this->mobile_title = '确认支付';
        }elseif ($lang == 2) {
            $this->mobile_title = 'lao确认支付';
        }else{
            $this->mobile_title = 'eng确认支付';
        }
        $this->display();
    }
