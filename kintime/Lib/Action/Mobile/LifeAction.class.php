...
class LifeAction extends CommonAction {
    ...
    public function cates($channel_id=0){
        foreach($this->lifechannel as $k=>$channel){
            if(empty($channel_id) || ($channel_id == $k)){
    ...
    public function channel() {
        ...
        $this->mobile_title = $this->lifechannel[$channel];
        $this->display(); // 输出模板
    }
    ...
    public function load() {
        ...
        if($keyword = $this->_param('keyword')){
    ...
    public function lists() {
        ...
        $this->assign('cat',$cat);
        $this->mobile_title = $cate['cate_name'];
        $this->display(); // 输出模板
    }
