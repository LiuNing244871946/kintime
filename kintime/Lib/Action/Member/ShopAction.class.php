...
class ShopAction extends CommonAction {
    ...
    public function myshop() {
        ...
        $list = $shop->where($map)->order(array('mer_id' => 'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function tongji() {
        $shopIds = D('merchants')->getShopIdsByTuiId($this->uid);
        if (empty($shopIds)) {
        ...
        $shops = D('merchants')->itemsByIds($shopIds);
        $datas = D('Shopmoney')->sumByIdsTop10($bg_date, $end_date, $shopIds);
        $showdatas = array();
        foreach ($datas as $k => $val) {
            if (!empty($val['mer_id'])) {
                $showdatas['shop'][] = '"' . $shops[$val['mer_id']]['mer_name'] . '"';
    ...
    public function shoplist() {
        ...
        if ($keyword = $this->_param('keyword', 'htmlspecialchars')) {
            $map['mer_name|tel'] = array('LIKE', '%' . $keyword . '%');
    ...
    public function dianping($shop_id) {
        ...
        if ($this->isPost()) {
            ...
            if ($dianping_id = $obj->add($data)) {
                ...
                $this->baoMsg('评价成功', U('Pchome/shop/detail', array('shop_id' => $shop_id)));
            }
            $this->baoMsg('操作失败！');
        }else {
