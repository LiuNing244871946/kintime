...
class ShopAction extends CommonAction {
    ...
    public function detail() {
        ...
        $tuan = D('Tuan')->where(array('shop_id' => $shop_id, 'audit' => 1,'city_id'=>$this->city_id, 'closed' => 0, 'end_date' => array('EGT', NOW)))->order(' tuan_id desc ')->limit(0, 6)->select();
    ...
    public function coupon($shop_id){
        ...
        if ($detail['closed'] ==1 ||$detail['audit'] == 0) {
            $this->error('该商家不存在');
            die;
        }
        $coupon = D('Coupon');
        import('ORG.Util.Page'); // 导入分页类
        $map = array('shop_id' => $shop_id,'audit' => 1, 'closed' => 0,'city_id'=>$this->city_id, 'expire_date' => array('EGT', TODAY));
        if ($keyword = $this->_param('keyword', 'htmlspecialchars')) {
    ...
    public function huodong($shop_id){
         $shop_id = (int)$shop_id;
        if (!$detail = D('Shop')->find($shop_id)) {
            $this->error('没有该商家');
            die;
        ...
        $huodong = D('Activity');
        import('ORG.Util.Page'); // 导入分页类
        $map = array('shop_id' => $shop_id,'audit' => 1, 'closed' => 0,'city_id'=>$this->city_id, 'end_date' => array('EGT', TODAY),'bg_date' => array('ELT', TODAY));
        if ($keyword = $this->_param('keyword', 'htmlspecialchars')) {
            $map['title'] = array('LIKE', '%' . $keyword . '%');
            $this->assign('keyword', $keyword);
        }
        ...
        $show = $Page->show(); // 分页显示输出
        $list = $huodong->where($map)->order(array('activity_id' => 'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $this->assign('detail',$detail);
        $this->assign('list', $list); // 赋值数据集
        $this->assign('page', $show); // 赋值分页输出
        $this->display(); // 输出模板
    ...
    public function goods($shop_id){
        ...
        $goods = D('Goods');
        import('ORG.Util.Page'); // 导入分页类
        $map = array('shop_id' => $shop_id,'audit' => 1, 'closed' => 0,'city_id'=>$this->city_id, 'end_date' => array('EGT', TODAY));
        if ($keyword = $this->_param('keyword', 'htmlspecialchars')) {
            $map['title'] = array('LIKE', '%' . $keyword . '%');
            $this->assign('keyword', $keyword);
        }
    ...
    public function dianping() {
        ...
        $data['audit'] = $this->_CONFIG['site']['dpaudit'];
        $data['create_time'] = NOW_TIME;
        $data['create_ip'] = get_client_ip();
