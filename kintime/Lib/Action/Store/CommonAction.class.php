...
class CommonAction extends Action {
    ...
    protected function _initialize() {
        ...
        if (strtolower(MODULE_NAME) != 'passport' && strtolower(MODULE_NAME) != 'public') { //public 不受权限控制
            ...
            $this->merchants = D('Merchants')->find(array("where" => array('user_id' => $this->uid, 'closed' => 0, 'audit' => 1)));
            if (empty($this->merchants)) {
                $this->error('该用户没有开通商户', U('/mobile/index/index'));
            }
            if($this->merchants['closed'] ==1){
                $this->error('商家已关闭');
            }
            $this->mer_id = $this->merchants['mer_id']; //为了程序调用的时候方便
            $this->assign('merchants', $this->merchants);
        }
