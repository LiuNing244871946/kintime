...
class EleproductAction extends CommonAction {
    ...
    private $create_fields = array('product_name', 'lao_product_name', 'eng_product_name', 'cate_id', 'photo', 'price', 'is_new', 'is_hot', 'is_tuijian', 'create_time', 'create_ip');
    private $edit_fields = array('product_name', 'lao_product_name', 'eng_product_name', 'cate_id', 'photo', 'price', 'is_new', 'is_hot', 'is_tuijian');
    ...
    public function index() {
        ...
        if ($keyword = $this->_param('keyword', 'htmlspecialchars')) {
            $map['product_name|lao_product_name|eng_product_name'] = array('LIKE', '%' . $keyword . '%');
        ...
        $data = D('rebate')->select();
        $return = array();
        foreach($data as $val){
            $return[$val['id']] = $val;
        }
        $this->assign('rate', D('Shop')->where('mer_id='.$this->mer_id)->find()['rate']);
        $this->assign('rebate', $return);
    ...
    private function createCheck() {
        ...
        $data['lao_product_name'] = htmlspecialchars($data['lao_product_name']);
        $data['eng_product_name'] = htmlspecialchars($data['eng_product_name']);
        if (empty($data['product_name'])) {
            $this->baoError('中文菜名不能为空');
        } 
         if (empty($data['lao_product_name'])) {
            $this->baoError('老文菜名不能为空');
        } 
         if (empty($data['eng_product_name'])) {
    ...
    public function edit($product_id = 0) {
        if ($product_id = (int) $product_id) {
            ...
            if (!$detail = $obj->find($product_id)) {
                $this->baoError('请选择要编辑的菜品');
            }
            if($detail['shop_id'] != $this->shop_id){
                $this->baoError('请不要操作其他商家的菜品');
            }
            if ($this->isPost()) {
                $data = $this->editCheck();
