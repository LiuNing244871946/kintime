...
class TuanAction extends CommonAction {
    ...
    private $create_fields = array('shop_id','intro','lao_intro','eng_intro', 'title','lao_title','eng_title','limit', 'photo','tuan_price','num','bg_date', 'end_date','is_hot', 'is_new', 'is_chose', 'freebook');
    private $edit_fields = array('shop_id','intro','lao_intro','eng_intro', 'title','lao_title','eng_title','limit', 'photo','tuan_price','num','bg_date', 'end_date','is_hot', 'is_new', 'is_chose', 'freebook');
    ...
    public function order() {
        ...
        $ordercode = D('Tuancode')->itemsByorderIds($order_ids);
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($tuan_id = $obj->add($data)) {
                ...
                D('Tuandetails')->add(array('tuan_id' => $tuan_id, 'details' => $details,'lao_details' => $lao_details,'eng_details' => $eng_details, 'instructions' => $instructions, 'lao_instructions' => $lao_instructions, 'eng_instructions' => $eng_instructions));
    ...
    public function edit($tuan_id = 0) {
        if ($tuan_id = (int) $tuan_id) {
            ...
            if ($this->isPost()) {
                ...
                if ($words = D('Sensitive')->checkWords($details)) {
                    $this->baoError('商品中文详情有敏感词：' . $words);
                }
                $lao_details = $this->_post('lao_details', 'SecurityEditorHtml');
                if (empty($lao_details)) {
                    $this->baoError('商品老文详情不能为空');
                ...
                if ($words = D('Sensitive')->checkWords($instructions)) {
                 ...
                 $lao__instructions = $this->_post('lao_instructions', 'SecurityEditorHtml');
                if (empty($lao__instructions)) {
                    $this->baoError('老文购买须知不能为空');
                }
                if ($words = D('Sensitive')->checkWords($lao__instructions)) {
                    $this->baoError('老文购买须知含有敏感词：' . $words);
                }
                 $eng_instructions = $this->_post('eng_instructions', 'SecurityEditorHtml');
                if (empty($eng_instructions)) {
                ...
                if (false !== $obj->save($data)) {
                    D('Tuandetails')->save(array('tuan_id' => $tuan_id, 'details' => $details,'lao_details' => $lao_details,'eng_details' => $eng_details, 'instructions' => $instructions, 'lao_instructions' => $lao_instructions, 'eng_instructions' => $eng_instructions));
                    $this->baoSuccess('操作成功', U('tuan/index'));
                }
                $this->baoError('操作失败');
            } else {
