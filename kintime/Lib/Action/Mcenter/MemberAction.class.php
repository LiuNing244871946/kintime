...
class MemberAction extends CommonAction {
    ...
    protected function ding_success($message, $detail) {
        ...
        $order['date_time'] = date("Y-m-d H:i:s",$order['create_time']);
        $this->assign('menus',D('Shopdingmenu')->itemsByIds($menu_ids));
        $this->assign('shop',D('Shop')->find($order['shop_id']));
    ...
    protected function other_success($message, $detail) {
        ...
        if (!empty($tuanorder['branch_id'])) {
            ...
            $addr['cha'] = $branch['addr'];
            $addr['lao'] = $branch['lao_addr'];
            $addr['eng'] = $branch['eng_addr'];
        } else {
            $shop = D('Shop')->find($tuanorder['shop_id']);
            $addr['cha'] = $shop['addr'];
            $addr['lao'] = $shop['lao_addr'];
            $addr['eng'] = $shop['eng_addr'];
        }
    ...
    public function pay() {
        ...
        $detail = D('Paymentlogs')->find($logs_id);
        $order_id = $detail['order_id'];
