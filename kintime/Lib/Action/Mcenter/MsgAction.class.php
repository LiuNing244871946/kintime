...
class MsgAction extends CommonAction {
    ...
    public function mlistnew(){
        ...
        $message = D('Usermessage');
        import('ORG.Util.Page'); // 导入分页类
        $list1 = $message->where(array('user_id' => $this->uid, 'from_id' => $uid))->order(array('message_id' => 'asc'))->select();
        foreach ($list1 as $k => $val) {
            $list11[$val['message_id']] = $val;
            $list11[$val['message_id']]['send'] = 1;  //该用户发送消息给我
        }
        $list2 = $message->where(array('user_id' => $uid, 'from_id' => $this->uid))->order(array('message_id' => 'asc'))->select();
        ...
        $this->assign('from', D('Users')->find($uid));
        $this->assign('list', $list); // 赋值数据集
        $this->assign('page', $show); // 赋值分页输出
        $this->assign('uid', $uid);
        $this->mobile_title = '消息列表';
        $this->display();
    }
    ...
    public function reply(){
        ...
        if(!$detail = D('Users')->find($uid)){
            $error+=1;
        }
        $map = array('user_id'=>$this->uid,'from_id'=>$uid);
        $map['is_read'] = 0;
        ...
        if($error == 0 && !empty($list)){
            echo json_encode(array('status'=>'success','res'=>$list));
            D('Usermessage')->where(array('user_id'=>$this->uid,'from_id'=>$uid))->save(array('is_read'=>1));
            die;
        }else{
