...
class weixin {
    ...
    public function getCode($logs, $payment) {
        ...
        $input->SetScene_info('{"h5_info": {"type":"Wap","wap_url": "https://www.kintime.net","wap_name": "kintime"}}');
        $result = GetH5PayUrl($input);
        $redirect_url =urlencode(__HOST__ . U('mobile/payment/yes',array('log_id'=>$logs['logs_id'])));
        $url = $result["mweb_url"]."&redirect_url=".$redirect_url;  
        $str = '<a class="payment" type="button" href="'.$url.'">立即支付</a>';
