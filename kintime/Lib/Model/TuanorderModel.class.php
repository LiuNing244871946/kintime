...
class TuanorderModel extends CommonModel{
    ...
    public function money($bg_time,$end_time,$shop_id){      
        ...
        $shop_id = (int) $shop_id;
        if(!empty($shop_id)){
            $data = $this->query(" SELECT sum(price)/100 as price,FROM_UNIXTIME(`time`,'%m%d') as d from bao_all_order where `time` >= '{$bg_time}' AND `time` <= '{$end_time}' AND shop_id = '{$shop_id}'  group by  FROM_UNIXTIME(`time`,'%m%d')");   
        }else{
            $data = $this->query(" SELECT sum(price)/100 as price,FROM_UNIXTIME(`time`,'%m%d') as d from bao_all_order where `time` >= '{$bg_time}' AND `time` <= '{$end_time}'  group by  FROM_UNIXTIME(`time`,'%m%d')");      
        }
        $showdata = array();
        $days = array();
	...
	public function update_count($guoqi_time)
	{
		...
		if(D('Tuanorder')->where(array('order_id'=>array('in',$order_list)))->save(array('status'=>'0','update_time'=>$time))){
