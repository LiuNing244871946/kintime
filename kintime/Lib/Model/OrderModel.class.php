...
class OrderModel extends CommonModel {
    ...
    public function money($bg_time, $end_time, $shop_id) {
        ...
        $shop_id = (int) $shop_id;
        if (!empty($shop_id)) {
            $data = $this->query(" SELECT count(*) as price,FROM_UNIXTIME(`time`,'%m%d') as d from bao_all_order where time >= '{$bg_time}' AND time <= '{$end_time}' AND shop_id = '{$shop_id}'  group by  FROM_UNIXTIME(time,'%m%d')");
        } else {
            $data = $this->query(" SELECT count(*) as price,FROM_UNIXTIME(`time`,'%m%d') as d from bao_all_order where time >= '{$bg_time}' AND time <= '{$end_time}'  group by  FROM_UNIXTIME(time,'%m%d')");
        }
        $showdata = array();
        $days = array();
