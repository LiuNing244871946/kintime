...
class ShopdingModel extends CommonModel{
    ...
    public function items($filter=array(), $orderby=array())
    {
        ...
        if($type_id = (int)$filter['type_id']){
            $ext_table = "LEFT JOIN ".$this->tablePrefix.'shop_ding_attr'." b ON a.shop_id=b.shop_id";
			$ext_where = "b.type_id=$type_id";
        }
        unset($filter['type_id']);
        //$where = $this->where($filter, 'a.');
	...
	{
		$cfg = $this->getCfg();
		$detail = D('Shopding')->where(array('shop_id'=>$shop_id))->find();
